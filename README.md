...
- **月度预算**：设置每月花销上限（如 1500），报表页顶部实时显示本月「已花 / 剩余 / 超支警示」。
- **时段统计（寒暑假/学期）**：可在「我的」定义时间段（寒假/暑假/学期/自定义 + 起止日期），报表页切换统计范围到任意时段，并按**同类时段对比**历史支出。
...
- [x] **M6**：账单导入（微信/支付宝/银行 CSV，按金额+时间+商户去重）+ 月度预算（剩余花销）+ 时段统计（寒假/暑假/学期，同类对比）。库升 v3。
